...
//
// Motion for the V2 experience, ported from iOS AvafliV2Effects.swift: native
...
// Joe's ACTUAL Figma animation files (bundled GIFs) via AvafliV2GifBurst — see
// AvafliV2Gif.kt.
//

// MARK: - Confetti

internal enum class AvafliConfettiStyle { Celebration, Gold }
 ...
 */
@Composable
internal fun AvafliV2ConfettiField(
    modifier: Modifier = Modifier,
    style: AvafliConfettiStyle = AvafliConfettiStyle.Celebration,
    count: Int = 42,
    speed: Double = 1.0,
) {
    ...
    Canvas(modifier) {
        ...
        val palette = if (style == AvafliConfettiStyle.Celebration) celebrationPalette else goldPalette
        ...
        for (i in 0 until count) {
            ...
            val alpha = if (style == AvafliConfettiStyle.Gold) {
 ...
 */
@Composable
internal fun AvafliV2AnimatedCheckmark(
    modifier: Modifier = Modifier,
    lineWidth: Dp = 7.dp,
) {
 ...
 */
internal fun Modifier.avafliPulseGlow(accent: Color): Modifier = composed {
    val transition = rememberInfiniteTransition(label = "avafliPulseGlow")
 ...
 * [avafliPulseGlow] but fully STATIC — no breathing. Every moving element
 * (pulse, confetti, check draw) waits for the single reveal beat. Mirrors
 * iOS `.shadow(color: accent.opacity(0.75), radius: 10)`.
 */
internal fun Modifier.avafliStaticGlow(accent: Color): Modifier = drawBehind {
    val blur = 10.dp.toPx()
    val glowRadius = max(size.width, size.height) / 2f + blur * 2f
    val brush = Brush.radialGradient(
