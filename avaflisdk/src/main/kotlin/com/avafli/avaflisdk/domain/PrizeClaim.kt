 ...
 * (and the iOS `PrizeClaimBlock` in AvafliAPI.swift).
 *
 ...
 * unit-testable without any view machinery (mirrors iOS AvafliPrizeClaimForm).
 */
internal data class PrizeClaimForm(
    ...
    val isStep1Valid: Boolean
        get() = AvafliFieldValidation.isValidName(firstName) &&
            AvafliFieldValidation.isValidName(lastName) &&
            AvafliFieldValidation.isValidOptionalPhone(phone)
...
/** Date display for the winner card (mirrors iOS AvafliClaimDates). */
internal object AvafliClaimDates {
