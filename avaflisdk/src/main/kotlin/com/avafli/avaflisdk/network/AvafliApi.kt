...
import com.avafli.avaflisdk.domain.SdkBranding
import com.avafli.avaflisdk.domain.SdkConfig
import com.avafli.avaflisdk.domain.*
import com.avafli.avaflisdk.services.Logger
import kotlinx.serialization.json.*

/**
 * Avafli API client — all endpoint interactions.
 */
internal class AvafliApi(
    ...
    suspend fun registerDevice(
        ...
        val token = response["token"]?.jsonPrimitive?.contentOrNull
            ?: throw AvafliError.RegistrationFailed("Missing token in response")
        val refreshToken = response["refreshToken"]?.jsonPrimitive?.contentOrNull
            ?: throw AvafliError.RegistrationFailed("Missing refreshToken in response")
        val uuid = response["uuid"]?.jsonPrimitive?.contentOrNull
            ?: throw AvafliError.RegistrationFailed("Missing uuid in response")
