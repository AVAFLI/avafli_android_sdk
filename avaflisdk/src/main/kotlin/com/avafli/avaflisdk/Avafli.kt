...
import com.avafli.avaflisdk.ui.AvafliExperienceActivity
...
object Avafli {
    ...
    private var api: AvafliApi? = null
     ...
     */
    fun configure(configuration: AvafliConfiguration) {
    ...
    private fun autoPresentIfEligible() {
        ...
        if (activity.isFinishing || activity.isDestroyed || activity is AvafliExperienceActivity) return
    ...
    internal fun present(activity: Activity, callback: ((Result<DailyEntryGrant>) -> Unit)? = null) {
        ...
        if (isSuspended) {
            logger?.info("Publisher suspended; declining to present Avafli experience")
            callback?.invoke(Result.failure(AvafliError.ServiceUnavailable()))
            return
        }
    ...
    private suspend fun registerDeviceIfNeeded(context: Context) {
        ...
        val currentApi = api ?: throw AvafliError.NotInitialized()
        val apiKey = config?.apiKey ?: throw AvafliError.InvalidPublisherKey()
    ...
    private fun prewarmPublisherArt() {
        AvafliV2ImageWarmer.prewarm(cachedGiveaway?.prizeImageUrl)
        AvafliV2ImageWarmer.prewarm(cachedSdkConfig?.branding?.logoUrl)
    }
    ...
    private fun isSuspendedError(e: Throwable): Boolean {
        // Already mapped.
        if (e is AvafliError.ServiceUnavailable) return true

        // Structured server error: parse the JSON body for an error code/status field.
        if (e is AvafliError.ServerError) {
            val structured = try {
                val body = e.message ?: ""
